...
<div class="sig">
  ...
  <div style="text-align:center">
    <div>&nbsp;</div>
    <div style="margin-top:50px;font-weight:700">Xác nhận của phụ huynh</div>
  </div>
</div>
        ...
        <table className="text-xs border-collapse" style={{ minWidth: 'max-content' }}>
          ...
          <tbody>
            ...
            <tr className="bg-indigo-50/70">
              <td className="sticky left-0 z-10 bg-indigo-50 border border-indigo-200 text-center py-1" style={{ minWidth: 32 }}>
                <GraduationCap size={12} className="text-indigo-400 mx-auto" />
              </td>
              <td className="sticky z-10 bg-indigo-50 border border-indigo-200 px-3 py-1 text-xs font-semibold text-indigo-700 whitespace-nowrap" style={{ minWidth: 160, left: 32 }}>
                Giáo viên dạy
              </td>
              {days.map(d => {
                const date = padDate(d)
                ...
                return (
                  <td key={d} className="border border-indigo-100 bg-indigo-50 p-0" style={{ width: 28, minWidth: 28 }}>
                    <select
                      value={tInfo?.teacher_id || ''}
                      onChange={e => onSetTeacher(date, e.target.value || null)}
                      disabled={!hasStudents}
                      title={tInfo?.ho_ten || 'Chưa có GV'}
                      className="w-full h-7 text-center text-indigo-700 font-bold bg-transparent border-0 outline-none cursor-pointer disabled:opacity-30 disabled:cursor-default"
              ...
              <td colSpan={3} className="border border-indigo-100 bg-indigo-50" />
            </tr>
...
function generateTeacherPrintHTML({ teachers, data, month, daysInMonth }) {
  ...
  const rows = teachers.map((t, idx) => {
    ...
    const cells = days.map(d => {
      ...
      const dow_idx = new Date(date).getDay()
      const isWe = dow_idx === 0 || dow_idx === 6
      if (sessions.length > 0) total += sessions.length
...
<div style="margin-top:6px;font-size:7pt;color:#555"><b>Chú thích:</b> Ký hiệu = tên viết tắt lớp. · = không dạy. Tổng = tổng số lớp-buổi.</div>
<script>window.onload=()=>{ window.print(); }<\/script>
</body></html>`
}
...
function TeacherMatrix({ teachers, data, month, daysInMonth }) {
  ...
  // Tổng buổi per teacher
  const totals = useMemo(() => {
    const res = {}
    teachers.forEach(t => {
      let sum = 0
      days.forEach(d => { sum += (data[t.id]?.[padDate(d)] || []).length })
      res[t.id] = sum
    })
    return res
        ...
        <table className="text-xs border-collapse" style={{ minWidth: 'max-content' }}>
          ...
          <tfoot>
            <tr className="bg-indigo-50">
              <td colSpan={2} className="sticky left-0 z-10 bg-indigo-50 border border-indigo-200 px-3 py-1 text-xs font-semibold text-indigo-700">Tổng buổi / ngày</td>
              {days.map(d => {
                const date = padDate(d)
                const total = teachers.reduce((s, t) => s + (data[t.id]?.[date]?.length || 0), 0)
                return (
                  <td key={d} className="border border-indigo-200 text-center font-bold text-indigo-700 text-xs py-1">
                    {total > 0 ? total : <span className="text-indigo-200">·</span>}
                  </td>
                )
              })}
              <td className="border border-blue-200 bg-blue-100 text-center font-bold text-blue-800 text-xs px-2">
                {teachers.reduce((s, t) => s + (totals[t.id] || 0), 0)}
              </td>
            </tr>
          </tfoot>
        </table>
        ...
        {teachers.map(t => {
          ...
          return (
            <div key={t.id} className="bg-gray-50 rounded-xl p-3">
              ...
              <p className="text-lg font-bold text-indigo-700 mt-0.5">{total} <span className="text-xs font-normal text-gray-400">buổi</span></p>
        ...
        <>
          {!selectedClass ? (
            <div className="bg-white rounded-2xl py-20 text-center text-gray-400 text-sm shadow-sm border border-gray-100">
              ...
              Chọn lớp để xem bảng điểm danh tháng
            </div>
          ) : loading ? (
